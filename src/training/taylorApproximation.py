...
for r in [1,3]:
    ...
    for n in tqdm(range(100,1100,100)):
        os.makedirs(f"./models/taylorApproximation/radius={r}/samples={n:04}")
        for s in range(1,11):
            ...
            file_name = f"./models/taylorApproximation/radius={r}/samples={n:04}/order={s:02}__width={m:04}"

            history_dataframe.to_json(file_name + ".json")
            model.save(file_name)

    for m in tqdm(range(100,1100,100)):
        os.makedirs(f"./models/taylorApproximation/radius={r}/width={m:04}")
        for s in range(1,11):
            n = 500
            dataframe = pd.read_csv(f"./data/radius={r}/order={s:02}/samples={n:04}")
            ...
            file_name = f"./models/taylorApproximation/radius={r}/width={m:04}/order={s:02}__samples={n:04}"
